...
async function selectShopData(selectValue) {
  ...
  let selectedShops = []; // Array to keep track of selected shops
  ...
  if (selectValue == "all") {
    ...
    for (let i = 0; i < query_data.rowCount; i++) {
      ...
      // Check if the shop has already been selected
      if (!selectedShops.includes(shopId)) {
        selectedShops.push(shopId); // Add the shop to the list of selected shops
